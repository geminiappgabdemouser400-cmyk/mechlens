"""Unit tests for CohereArchitectureAdapter: cfg, components, weight conversions, preprocess."""
...
def _make_cfg(
    ...
    """Minimal TransformerBridgeConfig for Cohere adapter tests."""
    ...
    # logit_scale/rope_parameters are dynamic attrs on cfg (not declared on the dataclass).
...
class TestCohereLogitScaleNoneCheck:
    """logit_scale=None falls back to default; explicit values are preserved (HF type is float | None)."""
...
class TestCohereAdapterComponentMapping:
    """component_mapping has the correct bridge types and HF module paths."""
    ...
    def test_rotary_emb_is_rotary_bridge(self, adapter: CohereArchitectureAdapter) -> None:
        # rotary_emb is top-level (not inside blocks), matching Llama/Falcon.
        assert adapter.component_mapping is not None
...
class TestCohereAdapterWeightConversions:
    ...
    def test_gqa_adapter_kv_heads_in_conversions(self) -> None:
        # K/V carry n=n_kv_heads; Q/O carry n=n_heads.
        ...
        assert _n("blocks.{i}.attn.q.weight") == 8
        assert _n("blocks.{i}.attn.k.weight") == 2
        assert _n("blocks.{i}.attn.v.weight") == 2
        assert _n("blocks.{i}.attn.o.weight") == 8
...
class TestCohereArchitectureGuards:
    ...
    def test_no_norm_offset_conversions(self, adapter: CohereArchitectureAdapter) -> None:
        # Cohere is not Gemma - no +1 norm offset entries.
        for key in adapter.weight_processing_conversions:
            assert "ln1" not in key
            assert "ln2" not in key
            assert "ln_final" not in key

    def test_no_mlp_weight_conversions(self, adapter: CohereArchitectureAdapter) -> None:
        for key in adapter.weight_processing_conversions:
            assert "mlp" not in key

    def test_block_is_parallel_block_bridge(self, adapter: CohereArchitectureAdapter) -> None:
        # Parallel attn+MLP: must be ParallelBlockBridge, NOT sequential BlockBridge.
        blocks = adapter.component_mapping["blocks"]
        assert isinstance(blocks, ParallelBlockBridge)

    def test_uses_rms_norm_false_guard(self, adapter: CohereArchitectureAdapter) -> None:
        # CohereLayerNorm is true mean-subtracting LayerNorm - guard against borrowing Llama's RMS.
