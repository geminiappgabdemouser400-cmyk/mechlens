...
- split_qkv_matrix interleaved QKV partition correctness
"""
...
# ---------------------------------------------------------------------------
# split_qkv_matrix - interleaved QKV partition
# ---------------------------------------------------------------------------


class TestNeoxSplitQkvMatrix:
    """split_qkv_matrix correctly partitions the interleaved [Q_h0,K_h0,V_h0,...] format."""

    def _make_attention_component(self, n_heads: int, d_head: int, d_model: int) -> SimpleNamespace:
        """Build a mock attention layer with a known interleaved QKV weight and bias."""
        total_out = n_heads * 3 * d_head
        qkv_weight = torch.arange(total_out * d_model, dtype=torch.float32).reshape(
            total_out, d_model
        )
        qkv_bias = torch.arange(total_out, dtype=torch.float32)
        qkv_linear = SimpleNamespace(weight=qkv_weight, bias=qkv_bias)
        return SimpleNamespace(query_key_value=qkv_linear)

    def test_weight_partition_matches_interleaved_format(self) -> None:
        """Q/K/V weight slices must follow the NeoX per-head interleaved layout."""
        n_heads, d_head, d_model = 4, 16, 64
        adapter = NeoxArchitectureAdapter(_make_cfg(n_heads=n_heads, d_model=d_model))
        component = self._make_attention_component(n_heads, d_head, d_model)

        W = component.query_key_value.weight.view(n_heads, 3 * d_head, d_model)
        expected_q = W[:, :d_head, :].reshape(n_heads * d_head, d_model)
        expected_k = W[:, d_head : 2 * d_head, :].reshape(n_heads * d_head, d_model)
        expected_v = W[:, 2 * d_head :, :].reshape(n_heads * d_head, d_model)

        q_lin, k_lin, v_lin = adapter.split_qkv_matrix(component)

        assert torch.equal(q_lin.weight.data, expected_q)
        assert torch.equal(k_lin.weight.data, expected_k)
        assert torch.equal(v_lin.weight.data, expected_v)

    def test_bias_partition_matches_interleaved_format(self) -> None:
        """Q/K/V bias slices must follow the NeoX per-head interleaved layout."""
        n_heads, d_head, d_model = 4, 16, 64
        adapter = NeoxArchitectureAdapter(_make_cfg(n_heads=n_heads, d_model=d_model))
        component = self._make_attention_component(n_heads, d_head, d_model)

        b = component.query_key_value.bias.view(n_heads, 3 * d_head)
        expected_bq = b[:, :d_head].reshape(n_heads * d_head)
        expected_bk = b[:, d_head : 2 * d_head].reshape(n_heads * d_head)
        expected_bv = b[:, 2 * d_head :].reshape(n_heads * d_head)

        q_lin, k_lin, v_lin = adapter.split_qkv_matrix(component)

        assert torch.equal(q_lin.bias.data, expected_bq)
        assert torch.equal(k_lin.bias.data, expected_bk)
        assert torch.equal(v_lin.bias.data, expected_bv)
